...
var generateUuid = __webpack_require__(6).uuid;
...
var networkInterfaces = __webpack_require__(5).networkInterfaces();
...
function () {
  function Channel(channelName, options) {
    ...
    this._msgReceiver.on('error', function (err) {
      ...
      _this._internalEvent.emit('error', err);
    });
  }
  ...
  (0, _createClass2.default)(Channel, [{
    ...
    key: "onReconnect",
    value: function onReconnect(listener) {
      if (!_.isFunction(listener)) {
        throw new TypeError('Listener must be a function.');
      }
    ...
    key: "connect",
    value: function connect(isReconnect) {
      var _this3 = this;
      ...
      var cbStyle = _.isFunction(this._options.connectListener);

      return new Promise(function (resolve, reject) {
        _this3._socket = net.connect(socketOptions);

        _this3._socket.on('data', function (chunk) {
          _this3._msgReceiver.recv(chunk, _this3._socket);
        });

        _this3._socket.on('error', function (err) {
          _this3._needReconnect = true;

          _this3._internalEvent.emit('error', err);
        });

        _this3._socket.on('connect', function () {
          _this3._registerChannel().then(function () {
            _this3._connected = true;
            _this3._needReconnect = true;
            debug('client connected, _connected:', _this3._connected);

            if (isReconnect) {
              do {
                var sub = _this3._eventStore.sub.pop();

                if (!sub) {
                  break;
                }

                _this3.subscribe(sub.topic, sub.listener);
              } while (true);

              do {
                var pull = _this3._eventStore.pull.pop();

                if (!pull) {
                  break;
                }

                _this3.pull(pull.topic, pull.options, pull.listener);
              } while (true);
            }

            if (cbStyle) {
              _this3._options.connectListener(null, _this3);
            } else {
              resolve(_this3);
            }
          }).catch(function (err) {
            if (cbStyle) {
              _this3._options.connectListener(err, _this3);
            } else {
              reject(err);
            }
          });
        ...
        _this3._socket.on('close', function (err) {
          _this3._responseEvent.removeAllListeners();

          _this3._subEvent.removeAllListeners();

          _this3._pullEvent.removeAllListeners();

          _this3._connected = false;

          _this3._socket.destroy();

          _this3._socket.unref();
    ...
    value: function reconnect() {
      ...
      setTimeout(function () {
        ...
        _this4.connect(true).then(function () {
          _this4._internalEvent.emit('reconnect');
        });
    ...
    key: "disconnect",
    value: function disconnect(graceful) {
      var _this5 = this;
      ...
      if (graceful) {
        return new Promise(function (resolve, reject) {
          ...
          _this5._socket.once('end', function () {
            isClose = true;
            debug("Channel ".concat(_this5.name, " disconnected"));
            resolve();
            return;
          });
          ...
          setTimeout(function () {
            if (!isClose) {
              _this5._socket.destroy();

              _this5._socket.unref();

              debug("Channel ".concat(_this5.name, " disconnected forcelly"));
              resolve();
            }
          }, 1000);
    ...
    value: function request(target, topic) {
      ...
      return new Promise(function (resolve, reject) {
        try {
          ...
          msg.setTopic(topic);
          msg.setSource(_this6.name);
          msg.setTarget(target);
          msg.setContentType(contentType);
          msg.setPayload(data);
          var msgBuf = msg.getBuffer();

          _this6._socket.write(msgBuf); // get response data


          _this6._responseEvent.once(msg.getEventName(), function (resMsg) {
            resolve(resMsg);
          });
        } catch (err) {
          reject(err);
    ...
    value: function _serverRequest(topic) {
      ...
      return new Promise(function (resolve, reject) {
        try {
          ...
          msg.setTopic(topic);
          msg.setSource(_this7.name);
          msg.setTarget('');
          msg.setContentType(contentType);
          msg.setPayload(data);
          var msgBuf = msg.getBuffer();

          _this7._socket.write(msgBuf); // get response data


          _this7._responseEvent.once(msg.getEventName(), function (resMsg) {
            resolve(resMsg);
          });
        } catch (err) {
          reject(err);
    ...
    value: function response(topic, listener) {
      ...
      this._serverRequest('addResponseListener', {
        ...
        if (resMsg.isError('REGISTER_FAIL')) {
          _this9._requestEvent.removeAllListeners(topic);

          _this9._internalEvent.emit('error', new Error("Register pull listener for topic: ".concat(topic, " fail")));
        }
      });
    ...
    value: function push(target, topic, items) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this10._socket.write(msg.getBuffer(), 'utf8', function () {
          resolve();
        });
      });
    }
    ...
    value: function pull(topicArg, optionsArg, listenerArg) {
      ...
      this._pullEvent.on(topic, function (msg) {
        Promise.resolve(listener(msg)).then(function () {
          _this11._sendAck(msg.header.id, topic);
        });
      });

      return this._serverRequest('addPullListener', regPayload, 'json').then(function (resMsg) {
        if (resMsg.isError('REGISTER_FAIL')) {
          _this11._pullEvent.removeAllListeners(topic);

          _this11._internalEvent.emit('error', new Error("Register pull listener for topic: ".concat(topic, " fail")));
        } else {
          _this11._eventStore.pull.push({
            topic: topic,
            options: options,
            listener: listener
          });
    ...
    value: function publish(target, topic, data) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this12._socket.write(msg.getBuffer(), 'utf8', function () {
          resolve();
        });
      });
    }
    ...
    value: function subscribe(topic, listener) {
      ...
      return this._serverRequest('addSubscribeListener', regPayload, 'json').then(function (resMsg) {
        if (resMsg.isError('REGISTER_FAIL')) {
          _this13._subEvent.removeAllListeners(topic);

          _this13._internalEvent.emit('error', new Error("Register subscribe listener for topic: ".concat(topic, " fail")));
        } else {
          _this13._eventStore.sub.push({
            topic: topic,
            listener: listener
          });
        }
...
exports.connect = function () {
  ...
  var channelName = _.isNil(args[0]) ? '' : args[0].trim();
  ...
  var channel = new Channel(channelName, options);
