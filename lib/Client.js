...
var getSocketPath = __webpack_require__(5).getSocketPath;
...
/*#__PURE__*/
function () {
  function Channel(channelName, serverChannel, clientSocket) {
    var _this = this;

    (0, _classCallCheck2.default)(this, Channel);
    // public properties
    this.channel = channelName;
    this.serverChannel = serverChannel; // private properties
  ...
  (0, _createClass2.default)(Channel, [{
    ...
    value: function response(topic, listener) {
      ...
      this._serverRequest('addResponseListener', {
        ...
        if (resMsg.isError('REGISTER_FAIL')) {
          _this4._requestEvent.removeAllListeners(topic);

          _this4._errorEvent.emit('error', new Error("Register pull listener for topic: ".concat(topic, " fail")));
        }
      });
    ...
    value: function push(target, topic, items) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this5._socket.write(msg.getBuffer(), 'utf8', function () {
          resolve();
        });
      });
    }
    ...
    value: function pull(topicArg, optionsArg, listenerArg) {
      ...
      this._pullEvent.on(topic, function (msg) {
        Promise.resolve(listener(msg)).then(function () {
          _this6._sendAck(msg.header.id, topic);
        });
      });

      this._serverRequest('addPullListener', regPayload, 'json').then(function (resMsg) {
        if (resMsg.isError('REGISTER_FAIL')) {
          _this6._pullEvent.removeAllListeners(topic);

          _this6._errorEvent.emit('error', new Error("Register pull listener for topic: ".concat(topic, " fail")));
        }
      });
    ...
    value: function publish(target, topic, data) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this7._socket.write(msg.getBuffer(), 'utf8', function () {
          resolve();
        });
      });
    }
    ...
    value: function subscribe(topic, listener) {
      ...
      return this._serverRequest('addSubscribeListener', regPayload, 'json').then(function (resMsg) {
        if (resMsg.isError('REGISTER_FAIL')) {
          _this8._subEvent.removeAllListeners(topic);

          _this8._errorEvent.emit('error', new Error("Register subscribe listener for topic: ".concat(topic, " fail")));
        }
      });
      return this;
    }
...
function _registerChannel(channel, serverChannel) {
  return channel._serverRequest('register').then(function (msg) {
    if (msg.header.error) {
      var err = new Error('register channel fail.');
...
exports.connect = function () {
  ...
  if (typeof connectListener === 'function') {
    ...
    var channel = new Channel(channelName, serverChannel, socket);
    socket.on('connect', function () {
      _registerChannel(channel).then(function () {
