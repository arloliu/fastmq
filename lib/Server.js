...
class Server {
  ...
  _setupRequestHandlers() {
    ...
    this._requestHandlers.getChannels = (msg, res) => {
      ...
      if (!_.isString(channelName) || !_.isString(type) || type !== 'regexp' && type !== 'glob') {
...
class ChannelManager {
  ...
  _updateChannelCache(name) {
    ...
    this._channelCache.forEach((value, key, map) => {
      const regex = new RegExp(key); // debug(`ChannelManager._updateChannelCache, regex: ${regex.toString()}, name: ${name}`);
  ...
  findResponseTopic(pattern, topic) {
    ...
    const channels = this._findTopic('response', pattern, topic); // debug(`findResponseTopic name:${pattern} topic: ${topic}, results: ${channels.length}`);
