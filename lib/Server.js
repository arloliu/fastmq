...
var QueueManager = __webpack_require__(27);
...
var ChannelManager = __webpack_require__(29);
  ...
  (0, _createClass2.default)(Server, [{
    ...
    value: function _messageHandler(msg, rawBuf, socket) {
      ...
      if (msg.isRequest()) {
        ...
        var _res = new Response(msg, socket);

        if (!this._processInternalRequest(msg, socket)) {
          _res.setError(ErrorCode.TOPIC_NONEXIST);
    ...
    value: function _setupRequestHandlers() {
      ...
      this._requestHandlers.register = function (msg, res) {
        ...
        var socket = res.socket; // handle anonymous channel registeration

        if (srcChannel.length === 0 || srcChannel === '') {
          do {
            srcChannel = uniqid();
          } while (_this2._channels.has(srcChannel));
...
exports.create = function () {
  ...
  if (!_.isString(arguments.length <= 0 ? undefined : arguments[0])) {
    throw new TypeError('Invalid channel name, channel name must be a string type.');
  } else {
    options.name = arguments.length <= 0 ? undefined : arguments[0];
  }

  if (arguments.length > 1) {
    if (_.isNumber(arguments.length <= 1 ? undefined : arguments[1])) {
      // create(name, port[, host])
      options.port = _.toNumber(arguments.length <= 1 ? undefined : arguments[1]);

      if (arguments.length > 2 && _.isString(arguments.length <= 2 ? undefined : arguments[2])) {
        options.host = arguments.length <= 2 ? undefined : arguments[2];
      }
    }
  }
...
var globToRegExp = __webpack_require__(23);
...
function () {
  ...
  (0, _createClass2.default)(TaskQueue, [{
    key: "_updateSubChannelCache",
    value: function _updateSubChannelCache(name) {
      debug("TaskQueue._updateSubChannelCache, name: ".concat(name));

      this._subChannelCache.forEach(function (value, key, map) {
    ...
    value: function _getSubscribeChannels(regexp) {
      var cacheKey = regexp.source;

      var cache = this._subChannelCache.get(cacheKey);
...
function () {
  ...
  (0, _createClass2.default)(QueueManager, [{
    ...
    value: function processPublishTask(target, topic, payload, contentType) {
      var queue = this.get('sub', topic);
      var targetRegExp = globToRegExp(target);
      queue.enqueue(payload, targetRegExp, contentType);
      queue.process();
    }
  }, {
    key: "processPushTasks",
    value: function processPushTasks(target, topic, items, contentType) {
      var queue = this.get('pull', topic);
...
function () {
  ...
  (0, _createClass2.default)(ChannelManager, [{
    ...
    key: "_updateChannelCache",
    value: function _updateChannelCache(name) {
      debug("ChannelManager._updateChannelCache, name: ".concat(name));

      this._channelCache.forEach(function (value, key, map) {
        var regex = new RegExp(key);
    ...
    value: function find(nameGlob) {
      var re = globToRegExp(nameGlob);
      var cacheKey = re.source;

      var cache = this._channelCache.get(cacheKey);
    ...
    value: function _findTopic(type, nameGlob, topic) {
      ...
      if (cache) {
        ...
        for (var _key in this._channels) {
          if (!Object.hasOwnProperty.call(this._channels, _key)) {
            continue;
          }

          if (re.test(_key) || _key === nameGlob) {
            var _channel2 = this._channels[_key];
            var _handlers = _channel2.handlers;

            if (_handlers.hasOwnProperty(topic) && _handlers[topic].type === type) {
              // debug(`got channel ${key} by type: ${type}, topic: ${topic}`);
              channels.push(_channel2);
            }
          }
        }
      }
